...
const RewardsPerSecondUpdated = 0;
...
describeContractHardhatRevertBeforeEach('SafetyModuleV2 initial emitted events', init, (ctx: TestContext) => {
  ...
  it('Events emitted during staking', async () => {
    ...
    const stakesUniqueByBlockNumber = await getLogs('GlobalIndexUpdated');
    await expectLogsByName(
      'GlobalIndexUpdated',
  ...
  async function getLogs(eventName: EventName): Promise<Event[]> {
    const filter = ctx.safetyModule.filters[eventName]();
    return ctx.safetyModule.queryFilter(filter);
  }

  function expectLogs(events: Event[], eventName: EventName, expectedArgsList: {}[][]): void {
  ...
  async function expectLogsByName(eventName: EventName, expectedArgsList: {}[][]): Promise<void> {
    const events = await getLogs(eventName);
    return expectLogs(events, eventName, expectedArgsList);
  }

  function expectArraysEqual(actual: {}[], expected: {}[], message: string): void {
