...
export function addStakingTestCases(
  ...
  async function init(ctx: TestContext) {
    ...
    await ctx.axorToken.connect(staker1).transfer(
      ctx.deployer.address,
      await ctx.axorToken.balanceOf(staker1.address),
    );
    await ctx.axorToken.connect(staker2).transfer(
      ctx.deployer.address,
  ...
  describeContractHardhatRevertBeforeEach('SM1Staking', init, (ctx: TestContext) => {
    ...
    describe('stake', () => {
      ...
      it('User can successfully stake if epoch zero has started', async () => {
        ...
        expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(
    ...
    describe('requestWithdrawal', () => {
      ...
      it('Cannot request a withdrawal during blackout window', async () => {
        ...
        const currentRemain = Number(await ctx.safetyModule.getTimeRemainingInCurrentEpoch())
        const blackoutWindow = Number(await ctx.safetyModule.getBlackoutWindow())
    ...
    describe('withdrawStake', () => {
      ...
      it('Staker can request full balance and make multiple partial withdrawals', async () => {
        ...
        expect(await ctx.axorToken.balanceOf(fundsRecipient.address)).to.equal(withdrawAmount);
        expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(0);
        expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(
    ...
    describe('transfer', () => {
      ...
      it('User with staked balance for one epoch can transfer to another user and claim rewards', async () => {
        ...
        const balanceBeforeClaiming = await ctx.axorToken.balanceOf(staker1.address);
