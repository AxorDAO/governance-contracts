/**
 * SM1Staking test cases, wrapped in a function to allow parametrization.
 */
...
import { SafetyModuleV1, SafetyModuleV1__factory } from '../../../types';
import {
...
export function addStakingTestCases(
  ...
  async function init(ctx: TestContext) {
    ...
    // Transfer away any existing AXOR balance.
    ...
    await ctx.axorToken
      ...
      .transfer(
        ...
        await ctx.axorToken.balanceOf(staker1.address),
      );
    await ctx.axorToken
      .connect(staker2)
      .transfer(
        ctx.deployer.address,
        await ctx.axorToken.balanceOf(staker2.address),
      );
    ...
    (ctx: TestContext) => {
      ...
      describe('stake', () => {
        it('User cannot stake if epoch zero has not started', async () => {
          ...
          await expect(
            smBeforeEpochZero.stake(stakerInitialBalance),
        ...
        it('User can successfully stake if epoch zero has started', async () => {
          ...
          // `axorToken` should be transferred to SafetyModuleV1 contract, and user should be given an
      ...
      describe('requestWithdrawal', () => {
        it('User with nonzero staked balance can request a withdrawal after epoch zero has started', async () => {
          ...
          // `axorToken` should still be owned by SafetyModuleV1 contract, and user should still own an
          // equivalent amount of `SM1ERC20` tokens
          expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(0);
          expect(
            await ctx.axorToken.balanceOf(ctx.safetyModule.address),
          ...
          expect(
            await ctx.safetyModule.getTotalActiveBalanceCurrentEpoch(),
          ).to.equal(stakerInitialBalance);
          expect(
            await ctx.safetyModule.getActiveBalanceCurrentEpoch(
              staker1.address,
            ),
          ).to.equal(stakerInitialBalance);
          expect(
          ...
          expect(
            await ctx.safetyModule.getTotalActiveBalanceNextEpoch(),
          ).to.equal(0);
          expect(
            await ctx.safetyModule.getActiveBalanceNextEpoch(staker1.address),
          ).to.equal(0);
          expect(
            await ctx.safetyModule.getActiveBalanceNextEpoch(staker2.address),
          ).to.equal(0);
          // Expect the next inactive balance to have increased relative to the current inactive balance.
          expect(
            await ctx.safetyModule.getTotalInactiveBalanceCurrentEpoch(),
          ).to.equal(0);
          expect(
            await ctx.safetyModule.getInactiveBalanceCurrentEpoch(
              staker1.address,
            ),
          ).to.equal(0);
          expect(
          ...
          expect(
            await ctx.safetyModule.getTotalInactiveBalanceNextEpoch(),
          ).to.equal(stakerInitialBalance);
          expect(
            await ctx.safetyModule.getInactiveBalanceNextEpoch(staker1.address),
          ).to.equal(stakerInitialBalance);
          expect(
            await ctx.safetyModule.getInactiveBalanceNextEpoch(staker2.address),
          ).to.equal(0);
        });

        it('Cannot request a withdrawal during blackout window', async () => {
          await contract.stake(staker1, stakerInitialBalance);

          const currentRemain = Number(
            await ctx.safetyModule.getTimeRemainingInCurrentEpoch(),
          );
          const blackoutWindow = Number(
            await ctx.safetyModule.getBlackoutWindow(),
          );
          ...
          await expect(
            contract.requestWithdrawal(staker1, stakerInitialBalance),
          ).to.be.revertedWith(
            'SM1Staking: Withdraw requests restricted in the blackout window',
          );

          // `axorToken` should still be owned by SafetyModuleV1 contract, and user should still have an
          // equivalent amount of `SM1ERC20` tokens
          expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(0);
          expect(
            await ctx.axorToken.balanceOf(ctx.safetyModule.address),
        ...
        it('User with zero staked balance cannot request a withdrawal', async () => {
          await expect(
            contract.requestWithdrawal(staker1, 1),
          ).to.be.revertedWith(
      ...
      describe('withdrawStake', () => {
        it('Staker can request and withdraw full balance', async () => {
          ...
          // `axorToken` should be sent to fundsRecipient, SafetyModuleV1 contract should own nothing
          // and user should have 0 staked token balance
          expect(
            await ctx.axorToken.balanceOf(fundsRecipient.address),
          ).to.equal(stakerInitialBalance);
          expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(0);
          expect(
            await ctx.axorToken.balanceOf(ctx.safetyModule.address),
        ...
        it('Staker can request full balance and make multiple partial withdrawals', async () => {
          ...
          // `axorToken` should be sent to fundsRecipient, SafetyModuleV1 contract should own remainder
          expect(
            await ctx.axorToken.balanceOf(fundsRecipient.address),
          ...
          expect(
            await ctx.axorToken.balanceOf(ctx.safetyModule.address),
          ).to.equal(stakerInitialBalance - withdrawAmount);
      ...
      describe('withdrawMaxStake', () => {
        it('Staker can request and withdraw full balance', async () => {
          ...
          await contract.requestWithdrawal(staker1, stakerInitialBalance);
          await contract.elapseEpoch(); // increase time to next epoch, so user can withdraw funds
          await contract.withdrawMaxStake(
            staker1.address,
      ...
      describe('Transfer events', () => {
        it('Emits transfer events as expected', async () => {
          await expect(
            ctx.safetyModule.connect(staker1).stake(stakerInitialBalance),
          )
          ...
          await expect(
            ctx.safetyModule
              .connect(staker2)
              .withdrawStake(fundsRecipient.address, stakerInitialBalance),
          )
            .to.emit(ctx.safetyModule, 'Transfer')
      ...
      describe('claimRewards', () => {
        ...
        it('User with nonzero staked balance for one epoch but emission rate was zero cannot claim rewards', async () => {
          ...
          await expect(
            ctx.safetyModule
              ...
              .setRewardsPerSecond(emissionRate),
          )
            .to.emit(ctx.safetyModule, 'RewardsPerSecondUpdated')
            .withArgs(emissionRate);

          expect(
            await stakerSigner1.callStatic.claimRewards(fundsRecipient.address),
          ).to.equal(0);
        });
        ...
        it('Multiple users can stake, requestWithdrawal, withdrawStake, and claimRewards', async () => {
          ...
          expect(
            (
              await stakerSigner1.callStatic.claimRewards(staker1.address)
            ).toNumber(),
          ...
          expect(
            (
              await stakerSigner2.callStatic.claimRewards(staker2.address)
            ).toNumber(),
        ...
        it('User with nonzero staked balance does not earn rewards after distributionEnd', async () => {
          ...
          await contract.requestWithdrawal(
            staker1.address,
            stakerInitialBalance,
          );
      ...
      describe('transfer', () => {
        ...
        it('User with staked balance for one epoch can transfer to another user and claim rewards', async () => {
          ...
          const balanceBeforeClaiming = await ctx.axorToken.balanceOf(
            staker1.address,
          );
