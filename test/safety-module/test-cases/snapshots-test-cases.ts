...
export function addSnapshotsTestCases(
  ...
  async function init(ctx: TestContext) {
    ...
    await ctx.axorToken.transfer(staker1.address, 100_000_000);
    await ctx.axorToken.connect(staker1).approve(ctx.safetyModule.address, 100_000_000);

    staker1InitialTokenBalance = await ctx.axorToken.balanceOf(staker1.address);
  }
  ...
  describeContractHardhatRevertBeforeEach('Safety Module snapshots - getPowerAtBlock()', init, (ctx: TestContext) => {
    ...
    async function expectBalances(
      ...
      expect(await ctx.axorToken.balanceOf(user.address)).to.equal(tokenBalance);
