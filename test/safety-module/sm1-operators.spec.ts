...
describeContractHardhatRevertBeforeEach('SM1Operator', init, (ctx: TestContext) => {
  ...
  describe('Stake and claim operators', () => {
    ...
    it('The stake operator can withdraw stake on behalf of a user', async () => {
      ...
      expect(await ctx.axorToken.balanceOf(operator.address)).to.equal(largeBalance);
    });
    ...
    it('The claim operator can claim rewards on behalf of a user', async () => {
      ...
      const initialBalance = await ctx.axorToken.balanceOf(operator.address);
      ...
      const finalBalance = await ctx.axorToken.balanceOf(operator.address);
