...
describeContract('wind-down-safety-module-proposal', init, (ctx: TestContext) => {
  ...
  it('Existing stakers can unstake', async () => {
    ...
    const stakerAxorRewardsBeforeUnstake = await existingStakerSafetyModule.callStatic.claimRewards(existingStaker);
    ...
    // Request to withdraw all of the staked AXOR.
    ...
    // Ensure user has their AXOR tokens.
    expect(await ctx.axorToken.balanceOf(existingStaker)).to.equal(stakerBalanceBeforeUnstake.toString());
    ...
    await expect(existingStakerSafetyModule.claimRewards(existingStaker))
      ...
      .withArgs(existingStaker, existingStaker, stakerAxorRewardsBeforeUnstake);
  });
});
