...
describeContractHardhatRevertBeforeEach('SM1Slashing', init, (ctx: TestContext) => {
  ...
  describe('slash', () => {
    ...
    it('slashing zero does nothing', async () => {
      ...
      expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(stakerInitialBalance);
      expect(await ctx.axorToken.balanceOf(fundsRecipient.address)).to.equal(0);
    ...
    it('slashes one token', async () => {
      ...
      // Check underlying token balances.
      expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(1);
      expect(await ctx.axorToken.balanceOf(fundsRecipient.address)).to.equal(1);
    ...
    it('does not affect stake balance, but affects gov power and tokens received on withdrawal', async () => {
      ...
      // Check underlying token balances.
      expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(0);
      expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(remainingAmount);
      expect(await ctx.axorToken.balanceOf(fundsRecipient.address)).to.equal(slashAmount);
      ...
      // Check underlying token balances.
      expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(remainingAmount);
      expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(0);
      expect(await ctx.axorToken.balanceOf(fundsRecipient.address)).to.equal(slashAmount);
  ...
  describe('max slash', () => {
    ...
    it('slashes 95%', async () => {
      ...
      // Check underlying token balances.
      expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(0);
      expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(stakerInitialBalance / 20);
      expect(await ctx.axorToken.balanceOf(fundsRecipient.address)).to.equal(stakerInitialBalance / 20 * 19);
      ...
      expect(await ctx.axorToken.balanceOf(staker1.address)).to.equal(stakerInitialBalance / 20);
    });
    ...
    it('staker receive funds via transfer after being slashed', async () => {
      ...
      expect(await ctx.axorToken.balanceOf(ctx.safetyModule.address)).to.equal(stakerInitialBalance + stakerInitialBalance / 20);
  ...
  describe('exchange rate max', () => {
    ...
    it('supports max slash up to 34 times', async () => {
      // This test case requires that we mint far beyond the initial supply of AXOR.
      const mockAxorToken = await new MintableERC20__factory(ctx.deployer).deploy(
        'Mock axor',
        'AXOR',
        18,
      );
      ...
      await mockAxorToken.mint(ctx.deployer.address, BigNumber.from(10).pow(30));
      ...
      let balanceBefore = await mockAxorToken.balanceOf(staker2.address);
      await safetyModule_2.connect(staker2).withdrawMaxStake(staker2.address);
      let balanceAfter = await mockAxorToken.balanceOf(staker2.address);
      ...
      // Stake again.
      await mockAxorToken.connect(staker2).approve(safetyModule_2.address, veryLargeAmountToStake);
      ...
      // Execute withdrawal.
      balanceBefore = await mockAxorToken.balanceOf(staker2.address);
      await safetyModule_2.connect(staker2).withdrawMaxStake(staker2.address);
      balanceAfter = await mockAxorToken.balanceOf(staker2.address);
