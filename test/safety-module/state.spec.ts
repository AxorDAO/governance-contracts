...
describeContractHardhatRevertBeforeEach('SafetyModuleV2 initial state', init, (ctx: TestContext) => {
  ...
  it('Staked token is set during contract creation', async () => {
    expect(await ctx.safetyModule.STAKED_TOKEN()).to.equal(ctx.axorToken.address);
  });

  it('Rewards token is set during contract creation', async () => {
    expect(await ctx.safetyModule.REWARDS_TOKEN()).to.equal(ctx.axorToken.address);
  });
  ...
  it('Emissions per second is initially zero', async () => {
    ...
    expect(await ctx.safetyModule.getRewardsPerSecond()).to.equal(0);
  });
