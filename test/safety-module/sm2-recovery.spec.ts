...
describeContractHardhatRevertBeforeEach('SM2Recovery', init, (ctx: TestContext) => {
  ...
  it('Receives tokens from the Safety Module', async () => {
    ...
    const balance = await ctx.axorToken.balanceOf(contract.address);
  ...
  it('Affected stakers can claim the owed amount', async () => {
    for (const stakerAddress of testStakers) {
      ...
      if ((await ctx.axorToken.balanceOf(stakerAddress)).lt(IMPERSONATED_ACCOUNT_STIPEND)) {
      ...
      const balanceBefore = await ctx.axorToken.balanceOf(stakerAddress);
      await contractAsStaker.claim();
      const balanceAfter = await ctx.axorToken.balanceOf(stakerAddress);
    ...
    // Verify the contract balance in the “full test” case with all stakers.
    if (testAllStakers) {
      const endingContractBalance = await ctx.axorToken.balanceOf(contract.address);
