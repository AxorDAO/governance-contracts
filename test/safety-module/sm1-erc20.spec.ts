...
async function init(ctx: TestContext) {
  ...
  mockSubclassSigner1 = await new MockSafetyModuleSubclass__factory(
    staker1,
  ).deploy(
  ...
  (ctx: TestContext) => {
    ...
    describe('allowance', () => {
      it('allowance is initially zero for user', async () => {
        expect(
          await ctx.safetyModule.allowance(staker1.address, staker2.address),
        ).to.equal(0);
      });
      ...
      it('cannot approve to address(0)', async () => {
        await expect(
          ctx.safetyModule.approve(ZERO_ADDRESS, 1),
        ).to.be.revertedWith('SM1ERC20: Approve to address(0)');
      });

      it('cannot approve from address(0)', async () => {
        await expect(
          mockSubclassSigner1.mockApproveFromZero(staker1.address, 1),
        ).to.be.revertedWith('SM1ERC20: Approve from address(0)');
      });
    });
    ...
    describe('increaseAllowance', () => {
      it('allowance can be increased', async () => {
        await expect(
          stakerSigner1.increaseAllowance(
            staker2.address,
            stakerInitialBalance,
          ),
        )
    ...
    describe('decreaseAllowance', () => {
      it('allowance can be decreased', async () => {
        ...
        await expect(
          stakerSigner1.decreaseAllowance(
            staker2.address,
            stakerInitialBalance,
          ),
        )
    ...
    describe('transfer', () => {
      it('User cannot transfer funds if they have not staked', async () => {
        expect(
          await ctx.safetyModule.getTransferableBalance(staker1.address),
        ).to.equal(0);
        await expect(
      ...
      it('cannot transfer to address(0)', async () => {
        await expect(
          ctx.safetyModule.transfer(ZERO_ADDRESS, 1),
        ).to.be.revertedWith('SM1ERC20: Transfer to address(0)');
      });

      it('cannot transfer from address(0)', async () => {
        await expect(
          mockSubclassSigner1.mockTransferFromZero(staker1.address, 1),
        ).to.be.revertedWith('SM1ERC20: Transfer from address(0)');
      });
    });
