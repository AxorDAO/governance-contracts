...
describeContract(
  'update-merkle-distributor-rewards-parameters-dip24',
  init,
  (ctx: TestContext) => {
    it('Proposal IPFS hash is correct', async () => {
      const proposalId = await readIdFromProposal(
        'updateMerkleDistributorRewardsParametersDIP24Proposal',
      );
    ...
    it('Merkle distributor reward parameters are updated for DIP 24', async () => {
      const [lpRewardsAmount, traderRewardsAmount, alphaParameter]: [
        BigNumber,
        BigNumber,
        BigNumber,
      ...
      expect(traderRewardsAmount.toString()).to.equal(
        '1582192000000000000000000',
      );
