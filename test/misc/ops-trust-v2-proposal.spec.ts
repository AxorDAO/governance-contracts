...
describeContractHardhatRevertBefore(
  'ops-trust-v2-proposal',
  init,
  (ctx: TestContext) => {
    it('Proposal IPFS hash is correct', async () => {
      const opsTrustV2ProposalId = await readIdFromProposal(
        'OpsTrustViaProposalV2',
      );
    ...
    it('DOT 2.0 receives tokens from the community treasury', async () => {
      const balance = await ctx.axorToken.balanceOf(
        ctx.config.DOT_MULTISIG_ADDRESS_V2,
      );
