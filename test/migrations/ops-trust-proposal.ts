...
export async function fundOpsTrustViaProposal({
  axorTokenAddress,
  governorAddress,
  ...
  await fundCommunityTreasuryFromFoundationIfNecessary({
    axorTokenAddress,
    communityTreasuryAddress,
    minTreasuryBalance: deployConfig.DOT_FUNDING_AMOUNT,
  });
  ...
  if (config.FUND_OPS_TRUST_PROPOSAL_ID !== null) {
    proposalId = config.FUND_OPS_TRUST_PROPOSAL_ID;
  } else {
    log('Creating proposal');
    ({ proposalId } = await createOpsTrustProposal({
      proposalIpfsHashHex: DIP_18_IPFS_HASH,
      axorTokenAddress,
      governorAddress,
      shortTimelockAddress,
      communityTreasuryAddress,
    ...
    log('Waiting for voting OpsTrustViaProposal to begin');
    writeIdProposal({ id: proposalId.toNumber(), proposal: 'OpsTrustViaProposal' });
  ...
  log('\n=== OPS TRUST FUNDING COMPLETE ===\n');
}

export async function fundOpsTrustNoProposal({
  axorTokenAddress,
  shortTimelockAddress,
  communityTreasuryAddress,
  dotMultisigAddress,
  ...
  await waitForTx(
    await communityTreasury.transfer(
      axorTokenAddress,
      dotMultisigAddress,
      deployConfig.DOT_FUNDING_AMOUNT,
    ),
  );
