...
export async function executeUpgradeGovernanceStrategyV2ViaProposal({
  ...
  if (config.UPGRADE_GOVERNANCE_STRATEGY_PROPOSAL_ID !== null) {
    ...
    log('Waiting for voting createUpgradeGovernanceStrategyV2Proposal to begin');
    writeIdProposal({ id: proposalId.toNumber(), proposal: 'createUpgradeGovernanceStrategyV2Proposal' });
...
export async function executeUpgradeGovernanceStrategyV2NoProposal({
  ...
  const governor = new AxorGovernor__factory(mockLongTimelock).attach(governorAddress);
