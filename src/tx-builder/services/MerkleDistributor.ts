...
import { AxorToken__factory, MerkleDistributorV1__factory } from '../../../types';
...
export default class MerkleDistributor extends BaseService<MerkleDistributorV1> {
  ...
  public async getTransfersRestrictedBefore(): Promise<number> {
    if (!this._transfersRestrictedBefore) {
      ...
      const axorToken: AxorToken = AxorToken__factory.connect(rewardsTokenAddress, provider);

      const transfersRestrictedBefore: BigNumber = await axorToken._transfersRestrictedBefore();
