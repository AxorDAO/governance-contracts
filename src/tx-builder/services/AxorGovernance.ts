...
import { AxorGovernor } from '../../../types/AxorGovernor';
import { Executor } from '../../../types/Executor';
import { GovernanceStrategy } from '../../../types/GovernanceStrategy';
import {
  axorGovernanceAddresses,
...
export default class AxorGovernanceService extends BaseService<AxorGovernor> {
  readonly axorGovernanceAddress: tEthereumAddress;

  readonly axorGovernanceStrategyAddress: tEthereumAddress;
  ...
  constructor(
    ...
    super(config, AxorGovernor__factory);
    ...
    this.axorGovernanceAddress = AXOR_GOVERNANCE;
  ...
  public async signVoting(
    ...
    const typeData = {
      ...
      domain: {
        name: 'Axor Governance',
        chainId: ChainId[this.config.network],
        verifyingContract: this.axorGovernanceAddress,
      },
      message: {
        support,
        id: proposalId,
  ...
  public async getLatestProposals(limit: number): Promise<Proposal[]> {
    const governance: AxorGovernor = this.getContractInstance(
      this.axorGovernanceAddress,
    );
  ...
  public async getGovernanceVoters(
    endBlock: number,
    startBlock: number = AXOR_GOVERNOR_DEPLOYMENT_BLOCK,
  ): Promise<Set<string>> {
    const governor: AxorGovernor = this.getContractInstance(
      this.axorGovernanceAddress,
    );
