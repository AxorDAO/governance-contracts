...
export async function deployPhase2({
  ...
  if (startStep <= 13) {
    ...
    await waitForTx(
      await rewardsTreasury.approve(axorTokenAddress, safetyModuleAddress, MAX_UINT_AMOUNT),
    );
    await waitForTx(
      await rewardsTreasury.approve(axorTokenAddress, merkleDistributorAddress, MAX_UINT_AMOUNT),
    );
    await waitForTx(
      await rewardsTreasury.approve(axorTokenAddress, liquidityStakingAddress, MAX_UINT_AMOUNT),
    );
  }
