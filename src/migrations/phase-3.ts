...
export async function deployPhase3({
  ...
  // Phase 2 deployed contracts.
  const rewardsTreasury = new Treasury__factory(deployer).attach(
    rewardsTreasuryAddress,
  );
  const rewardsTreasuryProxyAdmin = new ProxyAdmin__factory(deployer).attach(
    rewardsTreasuryProxyAdminAddress,
  );
  const safetyModule = new SafetyModuleV1__factory(deployer).attach(
    safetyModuleAddress,
  );
  const safetyModuleProxyAdmin = new ProxyAdmin__factory(deployer).attach(
    safetyModuleProxyAdminAddress,
  );
  const communityTreasury = new Treasury__factory(deployer).attach(
    communityTreasuryAddress,
  );
  const communityTreasuryProxyAdmin = new ProxyAdmin__factory(deployer).attach(
    communityTreasuryProxyAdminAddress,
  );
  const liquidityStaking = new LiquidityStakingV1__factory(deployer).attach(
    liquidityStakingAddress,
  );
  const liquidityStakingProxyAdmin = new ProxyAdmin__factory(deployer).attach(
    liquidityStakingProxyAdminAddress,
  );
  const merkleDistributor = new MerkleDistributorV1__factory(deployer).attach(
    merkleDistributorAddress,
  );
  const merkleDistributorProxyAdmin = new ProxyAdmin__factory(deployer).attach(
    merkleDistributorProxyAdminAddress,
  );
  ...
  if (startStep <= 3) {
    ...
    console.log(deployConfig.TRANSFERS_RESTRICTED_BEFORE);
    await waitForTx(
      await axorToken.updateTransfersRestrictedBefore(
  ...
  if (startStep <= 9) {
    log(
      'Step 9: Transfer rewards treasury proxy admin ownership to short timelock',
    );
    await waitForTx(
      await rewardsTreasuryProxyAdmin.transferOwnership(shortTimelock.address),
    );
  }

  if (startStep <= 10) {
    log(
      'Step 10: Transfer community treasury proxy admin ownership to short timelock',
    );
    await waitForTx(
      await communityTreasuryProxyAdmin.transferOwnership(
  ...
  if (startStep <= 11) {
    log(
      'Step 11: Transfer merkle distributor proxy admin ownership to short timelock',
    );
    await waitForTx(
      await merkleDistributorProxyAdmin.transferOwnership(
        shortTimelock.address,
      ),
    );
  }
  ...
  if (startStep <= 12) {
    log(
      'Step 12: Transfer liquidity staking proxy admin ownership to short timelock',
    );
    await waitForTx(
      await liquidityStakingProxyAdmin.transferOwnership(shortTimelock.address),
    );
  }

  if (startStep <= 13) {
    log(
      'Step 13: Transfer safety module proxy admin ownership to long timelock',
    );
    await waitForTx(
      await safetyModuleProxyAdmin.transferOwnership(longTimelock.address),
    );
  }
  ...
  if (startStep <= 14) {
    ...
    await waitForTx(
      await communityTreasury.transferOwnership(shortTimelock.address),
    );
  }
  ...
  if (startStep <= 15) {
    ...
    const txs = [
      ...
      await merkleDistributor.revokeRole(
      ...
      await liquidityStaking.revokeRole(
  ...
  if (startStep <= 19) {
    ...
    await waitForTx(
      await rewardsTreasury.transferOwnership(shortTimelock.address),
    );
  }

  // await expectDeployerBalance(
  //   axorToken,
  //   BigNumber.from(toWad(500_000_000)).sub(
  ...
  if (startStep <= 21) {
    log(
      'Step 21. Send tokens to be locked up in the community treasury vester',
    );
    await transferWithPrompt(
      axorToken,
      communityTreasuryVesterAddress,
  ...
  if (startStep <= 23) {
    log(
      'Step 23: Transfer AXOR token contract ownership to the short timelock',
    );
