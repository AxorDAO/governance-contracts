...
export async function deployPhase1({
  ...
  if (startStep <= 1) {
    ...
    axorToken = await new MockAxorToken__factory(deployer).deploy(
      deployerAddress,
    ...
    await waitForTx(axorToken.deployTransaction);
    axorTokenAddress = axorToken.address;
  } else {
    if (!axorTokenAddress) {
      throw new Error('Expected parameter axorTokenAddress to be specified.');
    }
    axorToken = new AxorToken__factory(deployer).attach(axorTokenAddress);
  }
